def run_auto():
    ...
    import pymysql,requests,operator
    # IMPORTS FOR LSTM
    ...
    def checkTableExists(c, Your_Table_Name):
        dbcur = c
        dbcur.execute("""
        SELECT COUNT(*)
        FROM information_schema.tables
        WHERE table_name = '{0}'
        """.format(Your_Table_Name.replace('\'', '\'\'')))
    ...
    # and then import all holdings from the 13F Filing Data from each fund 
    # into the table
    if(checkTableExists(c,Your_Table_Name) == False):
        Your_Table_Name = create_Table_and_Import.run_create_and_import()
        print("made a new table: " + Your_Table_Name)
    else: 
        print("" + Your_Table_Name + " aleady exists.")    
